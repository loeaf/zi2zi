...
class UNet(object):
    ...
    def build_model(self, is_training=True, inst_norm=False, no_target_source=False):
        ...
        # summary merge 부분도 수정
        d_merged_summary = tf.compat.v1.summary.merge([
            tf.compat.v1.summary.scalar("d_loss_real", d_loss_real),
            tf.compat.v1.summary.scalar("d_loss_fake", d_loss_fake),
            tf.compat.v1.summary.scalar("category_loss", category_loss),
            tf.compat.v1.summary.scalar("d_loss", d_loss)
        ])

        g_merged_summary = tf.compat.v1.summary.merge([
            tf.compat.v1.summary.scalar("cheat_loss", cheat_loss),
            tf.compat.v1.summary.scalar("l1_loss", l1_loss),
            tf.compat.v1.summary.scalar("fake_category_loss", fake_category_loss),
            tf.compat.v1.summary.scalar("const_loss", const_loss),
    ...
    def interpolate(self, source_obj, between, model_dir, save_dir, steps):
        ...
        embedding_vars = list(filter(filter_embedding_vars, tf.compat.v1.trainable_variables()))
